...
# 2022/07/08 : UNSD is using the UNIT_MULT from SDMX when presenting their data. We should convert in Mio to compare with other organisations.
#              (see Modifications lines 119 - 123 ) 
#
# 2021/06/25 : Ass the country name in the summary CSV file. 
#
# 2021/05/11 : Add a filter to each extraction of indidual organisation to remove the empty cell 
#              (see line 118, 152, 176, 204, 227)
...
TIME_PERIOD <- 2010:2021 #pay attention for new eurostat extraction 
...
# PA <- filter(UNSD, REF_AREA == "PA")
# UNSD is using the UNIT_MULT so we have to convert in Mio to get the right comparison point
UNSD$UNIT_MULT_NUMBER <- strtoi (UNSD$UNIT_MULT)
UNSD$obsValue <- UNSD$obsValue * (10 ** UNSD$UNIT_MULT_NUMBER) / (10 ** 6)
...
#temporary Cleaning of ECB
EU <- filter(EU, ref_area!="GB")
...
b<-Details_withPercent
to_save <- merge (a,b)
write.csv(to_save, "Details_withPercent.csv")
